...
products_info2 = requests.get('https://www.mvideo.ru/bff/products/prices', params=params, cookies=cookies, headers=headers).json()
products_info = requests.post('https://www.mvideo.ru/bff/product-details/list', cookies=cookies, headers=headers, json=json_data).json()

d1 = {}
d2 = {}
# print('статус код: ', response.status_code)
for i in products_info['body']['products']:  # названия моделей
    d1[i['productId']] = i['name']   # запись в словарь код товара + название
for i in products_info2['body']['materialPrices']:   # цена товара
...
for key, value in d1.items():
    for key2, value2 in d2.items():
        if key == key2:     # сравниваем коды товара и выстраиваем по порядку
            products_list[value] = value2
        continue

result = collection.insert_one(products_list)
print(products_list)
